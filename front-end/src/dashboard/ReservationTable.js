...
function ReservationTable({ reservations, cancelHandler }) {
  ...
  // Render the reservation table comp. 
  if (reservations) {
    return (
        ...
        <table className="table">
          ...
          <tbody>
            {reservations && reservations.length === 0 ? (
              <tr>
                <td colSpan={7}>No Reservations for date specified..</td>
              </tr>
            ) : null}
                    ...
                    <td>
                      {reservation.status === "booked" ? (
                        <Link to={`/reservations/${reservation.reservation_id}/seat`}>
                          <button type="button" className="btn btn-outline-secondary"> Seat </button>
                        </Link>
                      ) : null}
                      {reservation.status === "booked" ? (
                      ...
                      {reservation.status === "booked" ? (
                        <button data-reservation-id-cancel={reservation.reservation_id} name="cancel" className="btn btn-outline-secondary" onClick={()=>cancelHandler(reservation.reservation_id)}> Cancel </button>
                      ) : null}
                    </td>
                  </tr>
                );
